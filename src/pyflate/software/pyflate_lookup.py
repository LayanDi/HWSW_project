...
--- OPTIMIZATION #1 (this file, ONLY change from the original) ---
find_next_symbol() originally scanned every entry in the Huffman table
linearly for every symbol decoded (O(table size) per symbol; measured via
cProfile as ~48.9% of total runtime on the original, cumulative time
1.088s of 2.223s, called 148,271 times). This version precomputes, once
per Huffman table (in populate_huffman_symbols()), a dict mapping
(bit_length, code_value) -> symbol for both the reversed and
non-reversed cases, plus the sorted list of distinct bit lengths that
actually occur in the table. find_next_symbol() then tries each distinct
bit length in increasing order and does an O(1) dict lookup at each
length, instead of a linear scan+equality-check over every table entry.

This is safe and gives IDENTICAL results because canonical Huffman codes
are prefix-free: for the true code length L, snooping L bits gives
exactly that codeword, and by the prefix-free property no other, shorter
codeword can accidentally match a prefix of it - so trying lengths in
increasing order and returning the first dict hit is exactly the same
decision procedure as the original bits-then-code sorted table scan.

move_to_front() is intentionally left UNCHANGED in this file - this is a
single, isolated first experiment. If this alone does not reach the
required improvement, move_to_front will be optimized separately as a
second, independently-measured experiment.
"""
...
class HuffmanTable(object):
    ...
    def populate_huffman_symbols(self):
        ...
        # OPTIMIZATION #1: precompute O(1) lookup dicts, keyed by
        # (bit_length, code_value), for both the reversed and
        # non-reversed cases used by find_next_symbol(); also the
        # sorted list of distinct bit lengths actually present.
        self._lookup_rev = {}
        self._lookup_fwd = {}
        for x in self.table:
            self._lookup_rev[(x.bits, x.reverse_symbol)] = x.code
            self._lookup_fwd[(x.bits, x.symbol)] = x.code
        self._bit_lengths = sorted(set(x.bits for x in self.table))
    ...
    def find_next_symbol(self, field, reversed=True):
        # OPTIMIZATION #1: try each DISTINCT bit length (typically a
        # handful, e.g. <= ~20) in increasing order, doing an O(1) dict
        # lookup at each length, instead of linearly scanning every
        # individual table entry (which could be hundreds/thousands).
        # Correct because canonical Huffman codes are prefix-free: the
        # first length at which the snooped bits match a known
        # codeword is guaranteed to be the true code.
        lookup = self._lookup_rev if reversed else self._lookup_fwd
        for bits in self._bit_lengths:
            snooped = field.snoopbits(bits)
            code = lookup.get((bits, snooped))
            if code is not None:
                field.readbits(bits)
                return code
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = "Pyflate benchmark (optimization 1: O(1) Huffman lookup only)"
