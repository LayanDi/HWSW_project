"""
N-body benchmark - UNROLLED + sqrt() OPTIMIZATION (optimization #2)

Based on pyperformance's bm_nbody (Computer Language Benchmarks Game).

Optimization #1 (manual loop unrolling, see nbody_unrolled_v1.py) removed
tuple-unpacking/iteration overhead but measured NOT statistically
significant (pyperf compare_to). Conclusion: the bottleneck is in the
arithmetic itself, not the loop/unpacking structure.

Optimization #2 (this file) targets that arithmetic directly:
    dx*dx + dy*dy + dz*dz) ** (-1.5)
uses Python's ** operator with a non-integer exponent, which calls the
general-purpose pow(). This is replaced with:
    d2 = dx*dx + dy*dy + dz*dz
    inv_d = 1.0 / sqrt(d2)          # == d2 ** -0.5
    mag = dt * inv_d * inv_d * inv_d # == dt * d2 ** -1.5
using math.sqrt (imported once as a local/global reference to avoid
repeated attribute lookup), which is cheaper than a general pow() call.
Same change applied to report_energy()'s ** 0.5.

Mathematically equivalent to the original (up to negligible floating
point rounding differences) -> report_energy() results should match the
original/unrolled version to a very close tolerance.
"""
import pyperf
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = "n-body benchmark (unrolled + sqrt optimization)"
